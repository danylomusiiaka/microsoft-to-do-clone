...
export default function Dashboard({ allTodos, userData }: { allTodos: Task[]; userData: User }) {
  ...
  return (
    <main className='md:p-12 md:pr-0 md:mt-0 mt-5 w-full'>
      ...
      <p className='my-3 mb-6 '>{userData.team ? `для команди ${userData.team}` : `для користувача ${userData.name}`}</p>
      <div className='flex space-x-4 mb-5'>
        <button className='px-4 py-2 rounded-md font-semibold shadow-md hover:opacity-90 active:translate-y-1 transform' style={{ backgroundColor: "var(--secondary-background-color)" }} onClick={() => setToggleStats("status")}>
          По статусам
        </button>
        <button className='px-4 py-2 rounded-md font-semibold shadow-md hover:opacity-90 active:translate-y-1 transform' style={{ backgroundColor: "var(--secondary-background-color)" }} onClick={() => setToggleStats("priority")}>
          По пріорітетності
        </button>
      </div>
        ...
        <>
          {allTodos.length > 0 ? (
            <section className='flex flex-wrap xl:space-x-6  md:space-y-0 space-y-3'>
              <div className='p-3 md:w-96 w-full space-y-3 shadow-md' style={{ backgroundColor: "var(--secondary-background-color)" }}>
                {chartData.labels.map((status, index) => (
                  <div key={index} className='flex justify-between space-x-2'>
                    <div className='flex items-center space-x-2'>
                      <span className='w-4 h-4 inline-block' style={{ backgroundColor: chartData.colors[index] }}></span>
              ...
              <div className='p-3 md:w-fit w-full space-y-5 mt-3 shadow-md' style={{ backgroundColor: "var(--secondary-background-color)" }}>
                <p className='text-xl mt-3'>Кругова діаграма</p>
                <DonutChart data={chartData.data} inputLabels={chartData.labels} backgroundColors={chartData.colors} />
              </div>
            </section>
          ) : (
